...
// Plovdiv: ONE feed from the Bulgarian National Access Point, 29 bus lines run
// by four private operators under the city's contract. No tram, no trolleybus
// — Plovdiv's trolleybuses were withdrawn in 2012 and the wires came down —
// so this is the family's plainest map: one mode, one colour, one graph.
...
// Exact-match repairs for poles a feed names badly. Plovdiv needs none: of
// 528 stop names only seventeen shout, and every one of them is an acronym
// the city itself shouts (ДКЦ the polyclinic, ПГЕЕ the technical school,
// МОЛ 1 the mall).
const NAME_FIX = {};
...
// ONE feed, ONE cfg.
//
// The Bulgarian NAP (sipbg.gov.bg) publishes Plovdiv's timetable as a single
// GTFS, rebuilt nightly, with shapes. Four operators share it — Меритранс,
// Автобусни превози Пловдив, Меритранс 2017 and КЗТ-Златанови — but they run
// one municipal network under one numbering, so there is nothing to tell
// apart and no prefix to invent.
//
// The feed carries no direction_id at all (every trip is blank), which the
// engine already expects: where direction_id cannot separate directions it
// keys them by headsign instead, and this feed fills headsigns on every trip.
const LBL = new Map();
const isRailTrunk = () => false;

const MODES = [{
  mode: 'bus', label: 'buses', graphMode: 'road',
  // the whole city fits in one 9 × 11 km tile
  osmFiles: ['data/osm/tiles/t1.json'],
  color: '#0059a9', colorDark: '#00294f',
  all: busAll, lines: busList.length ? busList : (busAll ? [] : ['20']),
  feeds: [
    { tag: 'pdv', dir: 'data/gtfs', routeTypes: ['3'], mapKey: (sn) => sn || null },
  ],
}];
